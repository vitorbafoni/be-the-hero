# Be-The-Hero
Projeto desenvolvido para fins acadêmicos visando praticar Node.js, React.js, MongoDB e API REST.

<h1 align="center">
  <img src="frontend/src/assets/logo.svg" alt="Be The Hero" width="120">
<br>
Be The Hero
</h1>

<p align="center">Projeto desenvolvido para fim acadêmico utilizando Node.js, React.js, MongoDB e API REST</p>

<p align="center">
  <a href="https://opensource.org/licenses/MIT">
    <img src="https://img.shields.io/badge/License-MIT-blue.svg" alt="License MIT">
  </a>
</p>

<div>
  <img src="prints/01 - Logon.png" alt="demo" height="500" width="100%">
  <br>
  <hr>
  <br>
  <img src="prints/02 - Register.png" alt="demo" height="500" width="100%">
  <br>
  <hr>
  <br>
  <img src="prints/03 - Home.png" alt="demo" height="500" width="100%">
  <br>
  <hr>
  <br>
  <img src="prints/04 - Criar caso.png" alt="demo" height="500" width="100%">
  <br>
  <hr>
  <br>
  <img src="prints/05 - Registrar caso.png" alt="demo" height="500" width="100%">
  <br>
  <hr>
  <br>
  <img src="prints/06 - Listagem do caso.png" alt="demo" height="500" width="100%">
</div>

## Features

Essa aplicação foi desenvolvida com o intuito de praticar Node.js e React.js.

- 💹 **Node Js** — Utilizado para criar o backend da API.
- ⚛️ **React Js** — Utilizado para criar a interface do projeto.


## Getting started

1º Baixe ou clone o repositório: https://github.com/vitorbafoni/be-the-hero.git e abra o arquivo index.html<br>
2º Acesse a pasta "frontend" pelo terminal e digite "npm start".<br>
3º Acesse a pasta "backend" abrindo outro terminal e digite "npm start".


## License

This project is licensed under the MIT License - see the [LICENSE](https://opensource.org/licenses/MIT) page for details.
